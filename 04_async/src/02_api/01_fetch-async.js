/*
    ...
    기본 사용 방법
    let promise = fecch(ur1, [option]);

    url: 접근하고자 하는 url
    options: 선택 매개변수 http method나 headers, body 내용을 객체로 지정할 수 있다.

    option에 아무 값도 넣지 않으면 기본 get 메소드 (8가지 http method)로 요청하게 된다.
    ...
    fake api를 제공하고 RestAPI 방식으로 데이터를 호출 할 수 있는 경로
